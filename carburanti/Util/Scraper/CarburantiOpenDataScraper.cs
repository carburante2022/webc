...
internal static class CarburantiOpenDataScraper
{
    public static void Download()
    {
        ...
        foreach (var prezzo in records.Select(record => new Prezzo(record))) prezziGiorno.Aggiungi(prezzo);
        VarGlob.allData ??= new AllData();
        VarGlob.allData.AggiornaPrezzi(dateOnly, prezziGiorno);
    }
